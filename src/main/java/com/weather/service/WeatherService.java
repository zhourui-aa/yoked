...
public class WeatherService {
    ...
    /** Photon 地理编码（基于 OpenStreetMap，免费无需 Key，对中国城市覆盖极好） */
    ...
    /**
     * 调用 Photon Geocoding API（基于 OpenStreetMap），返回经纬度 + 国家。
     *
     * <p>Photon 对中国城市的覆盖远好于 Open-Meteo，支持所有地级市和大部分县级市。
     */
    private GeoResult geocode(String city) throws WeatherException {
        // Photon API：免费、无需 Key、对中国城市覆盖极好
        String url = GEOCODING_URL + "?q=" + encode(city) + "&limit=1";
        log("[调试] 地理编码: " + url);

        JsonObject root = httpGet(url);

        // Photon 返回 GeoJSON 格式：{"features": [{...}]}
        ...
        // GeoJSON 坐标格式：[longitude, latitude]（注意顺序！）
        ...
        if (!state.isEmpty() && !state.equals(name)) {
